...
export function buildSystemPrompt(
  ...
  return [
    ...
    `- Write tools (create_task, record_invoice_payment, add_lead_note, send_team_message) require user confirmation — call them when appropriate; the UI will ask to confirm.`,
    `- When the user sends a screenshot/image, carefully read visible text, UI labels, IDs (invoices, tickets), and errors, then use tools to act on what you see.`,
    `- To message teammates, search with search_team_members then send_team_message (confirm-gated). When the user @tags someone, use those IDs directly.`,
    `- @tags may include employees, CRM users, or customers/clients — treat tagged customer IDs as the customer context for lookups.`,
